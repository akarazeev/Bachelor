...
def create_file(user_id, file_name, data_frame, column_names, column_types):
    parameters = "host={} dbname={} user={} password={} port={}".format(str(HOST), str(DB_NAME), str(USER), str(PASS), str(PORT))
    query_for_insert = "INSERT INTO USERS_FILES(user_id, file_name) VALUES(%s, %s) RETURNING file_id"
    connection = psycopg2.connect(parameters)
    try:
        cursor = connection.cursor()
...
def get_files(user_id):
    parameters = "host={} dbname={} user={} password={} port={}".format(str(HOST), str(DB_NAME), str(USER), str(PASS), str(PORT))
    query = "SELECT users_files.file_id, users_files.file_name FROM USERS_FILES users_files JOIN USERS users ON users_files.user_id = users.user_id WHERE users_files.user_id = {}".format(str(user_id))
    connection = psycopg2.connect(parameters)
    try:
        df = pd.read_sql_query(query, connection)
        df.dropna(axis='index', how='any')
